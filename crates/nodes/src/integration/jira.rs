...
impl INodeType for JiraNode {
    ...
    async fn execute(
        ...
        for item_index in 0..run_count {
            ...
            let auth_token =
                resolve_auth_token(context, "Jira", item_index, "jiraApi", &["accessToken"])
                    .await?;
...
mod tests {
    ...
    #[tokio::test]
    async fn jira_uses_bound_credential_when_auth_token_is_empty() {
        let mut server = Server::new_async().await;
        let mock = server
            .mock("GET", "/rest/api/3/issue/PROJ-1")
            .match_header("authorization", "Bearer jira-token")
        ...
        context.add_param("baseUrl", json!(server.url()));
        context.add_param("issueKey", json!("PROJ-1"));
        context.add_credential("jiraApi", "accessToken", json!("jira-token"));
