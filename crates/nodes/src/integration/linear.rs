...
impl INodeType for LinearNode {
    ...
    async fn execute(
        ...
        for item_index in 0..run_count {
            ...
            let auth_token =
                resolve_auth_token(context, "Linear", item_index, "linearApi", &["accessToken"])
                    .await?;
...
mod tests {
    ...
    #[tokio::test]
    async fn linear_uses_bound_credential_when_auth_token_is_empty() {
        let mut server = Server::new_async().await;
        let mock = server
            .mock("POST", "/graphql")
            .match_header("authorization", "Bearer linear-token")
        ...
        context.add_param("operation", json!("listTeams"));
        context.add_param("baseUrl", json!(server.url()));
        context.add_credential("linearApi", "accessToken", json!("linear-token"));
