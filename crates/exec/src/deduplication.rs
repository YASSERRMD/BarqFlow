    ...
    pub fn filter_new_events(
        ...
        match mode {
            DeduplicationMode::ArrayOfIds => {
                ...
                if let Value::Object(map) = &previous_state.0 {
                ...
                for event in events {
                    if let Value::Object(map) = &event.0 {
                ...
                // Save back to state
                if let Value::Object(ref mut map) = previous_state.0 {
                    let mut arr = Vec::new();
                    for id in seen_ids {
...
mod tests {
    ...
    fn test_array_of_ids() {
        ...
        if let Value::Object(ref mut m) = e1.0 { m.insert("id".to_string(), json!("A")); }
        let mut e2 = IDataObject::default();
        if let Value::Object(ref mut m) = e2.0 { m.insert("id".to_string(), json!("B")); }
        ...
        if let Value::Object(ref mut m) = e3.0 { m.insert("id".to_string(), json!("C")); }
        ...
        if let Value::Object(m) = &new_evs2[0].0 {
            assert_eq!(m.get("id").unwrap().as_str().unwrap(), "C");
        }
    }
    ...
    fn test_incremented_key() {
        ...
        let mut events = Vec::new();
        let mut e1 = IDataObject::default();
        if let Value::Object(ref mut m) = e1.0 { m.insert("ts".to_string(), json!(100)); }
        let mut e2 = IDataObject::default();
        if let Value::Object(ref mut m) = e2.0 { m.insert("ts".to_string(), json!(200)); }
        ...
        if let Value::Object(ref mut m) = e1_5.0 { m.insert("ts".to_string(), json!(150)); }
        let mut e3 = IDataObject::default();
        if let Value::Object(ref mut m) = e3.0 { m.insert("ts".to_string(), json!(250)); }
        ...
        if let Value::Object(m) = &new_evs2[0].0 {
            assert_eq!(m.get("ts").unwrap().as_u64().unwrap(), 250);
        }
    }
}
