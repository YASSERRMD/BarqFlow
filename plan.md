# BarqFlow Complete Implementation Plan (Revised based on n8n Core)

This detailed plan breaks down the entire system into highly granular, independently verifiable phases. Each phase builds upon the previous one without cyclic dependencies. The tasks inside each phase represent the atomic commits that will be made to branch `phase_X_xyz`.
...
- [ ] Define `ItemId` and `RunId` UUIDs in `types.rs`
- [ ] Define `IBinaryData` struct matching n8n (mimeType, fileExtension, base64 payload / reference) in `types.rs`
...
- [ ] Add Rhai registered functions mapping to n8n built-ins (e.g., `$today`, `$now`)
...
## Phase 5: Executable Node Context (`crates/exec`)
*Purpose: Build the `IExecuteFunctions` concrete implementation that wraps a running node.*
...
- [ ] Implement `GetCredentials` interface mock (pending DB in later phase)
...
- [ ] Write unit tests for Context Parameter parsing with mock expressions

## Phase 6: Core Execution Loop (`crates/exec`)
...
- [ ] Implement Checkpointing hook system (for pausing/resuming async workflows)
...
## Phase 7: Node & Credential Registry (`crates/registry`)
*Purpose: The runtime map of all installed nodes. Mirrors n8n's `ICredentialTypes` and `INodeTypeRegistry`.*
...
- [ ] Implement versioning resolution logic (e.g., node v1 vs node v2)
...
- [ ] Write isolated execution tests for `Set` and `If` nodes

## Phase 9: Complex Standard Nodes (`crates/nodes`)
*Purpose: Implement nodes requiring external IO or advanced logic.*
...
## Phase 11: REST API & Authentication (`crates/api`)
